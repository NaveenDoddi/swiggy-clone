...
const url =
  "mongodb+srv://naveendoddi:zQTrjUrwyKXeIEZ2@swiggy.jbdpwef.mongodb.net/swiggy?retryWrites=true&w=majority&appName=swiggy";
...
app.get("/items/:email", async (req, res) => {
  // display items based on the mail for restraurants
  try {
    const email = req.params.email;
...
app.delete("/delCartItems/:id", async (req, res) => {
  // delete items form the cart page
  try {
...
app.delete("/cartEmpty/:mobile", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Items removed successfully.... cart empty" });
  } catch (err) {
    res.status(400);
  }
});
...
app.patch("/updateItem/:id", async (req, res) => {
  ...
  try {
    const updatedRestaurant = await items.findByIdAndUpdate(
      restaurantId,
      updates,
      { new: true },
    );
